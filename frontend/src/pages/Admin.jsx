...
const AdminDashboard = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/submissions/analytics/', {
        headers: {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 overflow-auto">
        <div className="bg-white rounded-lg w-full max-w-4xl p-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded">
              ...
              <div>
                ...
                <p className="font-medium capitalize">{selectedSubmission.domain}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedSubmission.mcq_answers.map((mcq, index) => (
                <div 
                  key={index}
                  className={`p-4 rounded ${mcq.is_correct ? 'bg-green-50' : 'bg-red-50'}`}
                >
                  <p className="font-medium">Question {mcq.question_id}</p>
                  <p className="text-gray-600">Selected: {mcq.answer}</p>
                  <p className={`text-sm ${mcq.is_correct ? 'text-green-600' : 'text-red-600'}`}>
          ...
          {/* Descriptive Section */}
          <div className="mb-6">
            <h3 className="text-lg font-semibold mb-2">Descriptive Responses</h3>
            {selectedSubmission.descriptive_answers.map((desc, index) => (
              <div key={index} className="mb-4 bg-gray-50 p-4 rounded">
                ...
                <p className="text-gray-600 whitespace-pre-wrap mb-2">{desc.answer}</p>
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-sm font-medium">{desc.score}/10</span>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="bg-gray-50 p-4 rounded">
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p className={`font-medium ${selectedSubmission.behavior_analysis.is_likely_bot ? 'text-red-600' : 'text-green-600'}`}>
                      {selectedSubmission.behavior_analysis.is_likely_bot ? 'Suspicious Activity' : 'Normal Activity'}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-600">Confidence</p>
                    <p className="font-medium">{selectedSubmission.behavior_analysis.confidence.toFixed(2)}%</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Key Presses</p>
                    <p className="font-medium">{selectedSubmission.behavior_analysis.total_key_presses}</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Backspaces</p>
                    <p className="font-medium">{selectedSubmission.behavior_analysis.backspace_count}</p>
                  </div>
                </div>
                {selectedSubmission.behavior_analysis.reasons?.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc pl-5">
                      {selectedSubmission.behavior_analysis.reasons.map((reason, index) => (
          ...
          {selectedSubmission.plagiarism?.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-2">Plagiarism Detection</h3>
                ...
                {selectedSubmission.plagiarism.map((plag, index) => (
                  <div key={index} className="bg-gray-50 p-4 rounded">
                    ...
                    <div className="flex items-center mt-1">
                      <div className="h-2 w-full bg-gray-200 rounded">
                        <div 
                          className={`h-2 rounded ${plag.confidence > 70 ? 'bg-red-500' : 'bg-yellow-500'}`}
                          style={{ width: `${plag.confidence}%` }}
                        ></div>
                      </div>
                      <span className="ml-2 text-sm font-medium">{plag.confidence}%</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {statistics && (
          <>
            <div className="bg-white p-4 rounded-lg shadow">
              <h2 className="text-gray-500 text-sm uppercase">Total Submissions</h2>
            ...
            {calculateDomainStats().map(stat => (
              <div key={stat.domain} className="bg-white p-4 rounded-lg shadow">
                <h2 className="text-gray-500 text-sm uppercase">{stat.domain}</h2>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Bot Risk</th>
